...
namespace o365calendar.Helpers
{
    internal class CalendarOperations
    {
        ...
        {
            var exchangeClient = await AuthenticationProvider.GetOutlookClientAsync(_calendarCapability);
            return await exchangeClient.Me.Calendar.Events.GetById(eventId).ExecuteAsync();
        }

        internal async Task<List<IEvent>> GetCalendarEventsAsync()
        {
            // Make sure we have a reference to the Exchange client
            var exchangeClient = await AuthenticationProvider.GetOutlookClientAsync(_calendarCapability);

            List<IEvent> returnResults = new List<IEvent>();
        ...
        {
            System.Diagnostics.Debug.WriteLine("Deleting event ...");
            try
            {
                // Make sure we have a reference to the Exchange client
                var exchangeClient = await AuthenticationProvider.GetOutlookClientAsync(_calendarCapability);
                ...
                System.Diagnostics.Debug.WriteLine("Event deleted");
                return true;
            }
            catch (Exception)
            {
                System.Diagnostics.Debug.WriteLine("Could not delete event");
            }
            return false;
        }
